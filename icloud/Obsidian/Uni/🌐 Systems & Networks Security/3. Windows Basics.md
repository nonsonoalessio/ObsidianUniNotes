...
Il principio di base di COM prevede che i client comunichino con gli oggetti (*COM server objects*) mediante l'uso di interfacce, cioè dei metodi logicamente collegati, raggruppati in una tabella virtuale di dispatch (*virtual table dispatch mechanism*), un metodo che utilizzano anche i compilatori C++ per implementare il dispatch di funzioni virtuali. Questo comporta la compatibilità binaria e la rimozione di problemi legati al compilatore (*compiler name mangling issue*), ed è possibile richiamare questi metodi da più linguaggi di programmazione.
